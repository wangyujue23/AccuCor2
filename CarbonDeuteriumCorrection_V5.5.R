...
NitrogenNaturalAbundace <- c(0.99632, 0.00368)#Used XCalibur parameter
OxygenNaturalAbundace <- c(0.99757, 0.00038, 0.00205)
SulfurNaturalAbundace <- c(0.9493, 0.0076, 0.0431)#Used XCalibur parameter
...
InputFile <- "CH_Compound_Test.xlsx"
...
IsotopeCorrection <- function(formula, datamatrix, label, Autofill = F, Charge = -1) {
  ...
  if((AtomNumber["C"]+1)*(AtomNumber["H"]+1) < length(label)){
    ...
    for(i in 1:(AtomNumber["H"]+1)){
      HPurityMatrix[i,] <- sapply(0:AtomNumber["H"], function(x) dbinom(x-i+1, x , 1-H2Purity))
    }
...
for (i in unique(InputDF$Compound)) {
  ...
  # OutputEnrichment <- rbind(OutputEnrichment, apply(CorrectedPercentage,2,Erh))
