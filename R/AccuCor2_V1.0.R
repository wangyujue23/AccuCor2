...
CH_Correction <- function(formula,
                          datamatrix,
                          label,
                          Resolution,
                          Autofill = F,
  ...
  MZ <-
    sum(AtomNumber * c(12, 1, 14, 16, 31, 32, 28, 35.5, 80)) / abs(Charge)
  Mass.Limit <- 1.66 * MZ^1.5 / Resolution / sqrt(ResDefAt)


  Mass.Difference <-
    (label$`13C#`) * 1.00628 + (label$`2H#`) * 1.00335
      ...
      if (Mass.Limit > abs((Mass.Difference[i] - Mass.Difference[j]) / Charge)) {
        print(
          paste(
            ...
            "-2H",
            label[i, 2],
            " is indistinguishable with 13C",
            label[j, 1],
            "-2H",
            label[j, 2],
            " under provided resolution, check your data again.",
            sep = ""
          )
        )
      }
  ...
  if ((AtomNumber["C"] + 1) * (AtomNumber["H"] + 1) < length(label)) {
    if (length(formula) != 0) {
      print(
        paste(
          formula,
          ":the number of labeling species exceeded the maximum allowed number of carbon and hydrogen isotopomers, check the input file."
        )
      )
    } else {
      print(
    ...
    for (i in 1:(AtomNumber["C"] + 1)) {
      CMatrix[, i] <-
        sapply(0:AtomNumber["C"], function(x) {
          stats::dbinom(x - i + 1, AtomNumber["C"] - i + 1, CarbonNaturalAbundance[2])
        })
    }
    for (i in 1:(AtomNumber["H"] + 1)) {
      HMatrix[, i] <-
        sapply(0:AtomNumber["H"], function(x) {
          stats::dbinom(x - i + 1, AtomNumber["H"] - i + 1, HydrogenNaturalAbundance[2])
        })
    }
    for (m in 1:(AtomNumber["H"] + 1)) {
      for (n in 1:m) {
        CHMatrix[((m - 1) * (AtomNumber["C"] + 1) + 1):(m * (AtomNumber["C"] + 1)), ((n - 1) * (AtomNumber["C"] + 1) + 1):(n * (AtomNumber["C"] + 1))] <-
          CMatrix * HMatrix[m, n]
      }
    }
    ...
    PurityMatrix <-
      diag((AtomNumber["C"] + 1) * (AtomNumber["H"] + 1))
    CPurityMatrix <- diag((AtomNumber["C"] + 1))
    HPurityMatrix <- diag((AtomNumber["H"] + 1))
    for (i in 1:(AtomNumber["C"] + 1)) {
      CPurityMatrix[i, ] <-
        sapply(0:AtomNumber["C"], function(x) {
          stats::dbinom(x - i + 1, x, 1 - C13Purity)
        })
    }
    for (i in 1:(AtomNumber["H"] + 1)) {
      HPurityMatrix[i, ] <-
        sapply(0:AtomNumber["H"], function(x) {
          stats::dbinom(x - i + 1, x, 1 - H2N15Purity)
        })
    }
    for (n in 1:(AtomNumber["H"] + 1)) {
      for (m in 1:n) {
        PurityMatrix[((m - 1) * (AtomNumber["C"] + 1) + 1):(m * (AtomNumber["C"] + 1)), ((n - 1) * (AtomNumber["C"] + 1) + 1):(n * (AtomNumber["C"] + 1))] <-
          CPurityMatrix * HPurityMatrix[m, n]
      }
    }
    ...
    Isotope.Combinations1 <- Isotope.Combinations %>%
      ...
      dplyr::filter((.data$O17 + .data$O18) <= AtomNumber["O"] &
        ...
        .data$NonTracerMass <= (AtomNumber["C"] + AtomNumber["H"])) %>%
      dplyr::filter((.data$NonTracerMass == .data$C13 + .data$H2)) %>%
      dplyr::mutate(
        MassDiff = 0.99703 * .data$N15
          + 1.00422 * .data$O17
          + 2.00425 * .data$O18
          + 0.99939 * .data$S33
          + 1.99580 * .data$S34
      ...
      dplyr::filter(abs(.data$MassDiff / Charge) < Mass.Limit)
    ...
    Isotope.Combinations2 <- Isotope.Combinations %>%
      ...
      dplyr::filter((.data$O17 + .data$O18) <= AtomNumber["O"] &
        ...
        (.data$Si29 + .data$Si30) <= AtomNumber["Si"] &
        .data$NonTracerMass <= (AtomNumber["C"] + AtomNumber["H"])) %>%
      dplyr::filter((.data$NonTracerMass == abs(.data$C13 - .data$H2)) &
        (.data$C13 * .data$H2 * .data$NonTracerMass > 0)) %>%
      dplyr::mutate(
        MassDiff =
          0.99703 * .data$N15
            + 1.00422 * .data$O17
            ...
            + 1.99705 * .data$Cl37
            + 1.99796 * .data$Br81
            - abs(1.00335 * .data$C13 - 1.00627 * .data$H2)
      ) %>%
      dplyr::filter(abs(.data$MassDiff / Charge) < Mass.Limit)

    NonTracerMatrix <- matrix(
      0,
      ncol = (AtomNumber["C"] + 1) * (AtomNumber["H"] + 1),
      nrow = (AtomNumber["C"] + 1) * (AtomNumber["H"] + 1)
    )
    for (m in 1:nrow(NonTracerMatrix)) {
      Cindex <- CompleteLabel[m, 1]
      Hindex <- CompleteLabel[m, 2]
      Current.Combinations <- Isotope.Combinations1 %>%
        dplyr::filter(.data$C13 == Cindex & .data$H2 == Hindex)
      if (nrow(Current.Combinations) != 0) {
        for (i in 1:nrow(Current.Combinations)) {
          p <-
            stats::dbinom(
              Current.Combinations[i, 3],
              AtomNumber["N"],
              NitrogenNaturalAbundance[2]
            ) *
              stats::dmultinom(unlist(c(
                AtomNumber["O"] - sum(Current.Combinations[i, 4:5]),
                Current.Combinations[i, 4:5]
              )), AtomNumber["O"], OxygenNaturalAbundance) *
              stats::dmultinom(unlist(c(
                AtomNumber["S"] - sum(Current.Combinations[i, 6:7]),
                Current.Combinations[i, 6:7]
              )), AtomNumber["S"], SulfurNaturalAbundance) *
              stats::dmultinom(unlist(c(
                AtomNumber["Si"] - sum(Current.Combinations[i, 8:9]),
                Current.Combinations[i, 8:9]
              )), AtomNumber["Si"], SiliconNaturalAbundance) *
              stats::dbinom(
                Current.Combinations[i, 10],
                AtomNumber["Cl"],
                ChlorineNaturalAbundance[2]
              ) *
              stats::dbinom(
                Current.Combinations[i, 11],
                AtomNumber["Br"],
    ...
    if (nrow(Current.Combinations) != 0) {
      for (i in 1:nrow(Current.Combinations)) {
        ...
        if (Cindex < Hindex) {
          ...
          m <-
            which(CompleteLabel$C13 == 0 & CompleteLabel$H2 == Hindex)
          for (j in 0:(AtomNumber["C"] + 1 - n)) {
            for (h in 0:(AtomNumber["H"] - m %/% (AtomNumber["C"] + 1))) {
              NonTracerMatrix[m + j + h * (AtomNumber["C"] + 1), n + j + h * (AtomNumber["C"] + 1)] <-
                NonTracerMatrix[m + j + h * (AtomNumber["C"] + 1), n + j + h * (AtomNumber["C"] + 1)] + p
            }
          }
        } else {
          n <- which(CompleteLabel$C13 == 0 & CompleteLabel$H2 == Hindex)
          m <-
            which(CompleteLabel$C13 == Cindex & CompleteLabel$H2 == 0)
...
#' @importFrom dplyr "%>%"
#' @return matrix of labeling pattern after the correction
CN_Correction <- function(formula,
                          datamatrix,
                          label,
                          Resolution,
                          Autofill = F,
  ...
  Mass.Difference <-
    (label$`13C#`) * 1.00628 + (label$`15N#`) * 0.99703
  for (i in 1:(length(Mass.Difference) - 1)) {
    for (j in (i + 1):length(Mass.Difference)) {
  ...
  if ((AtomNumber["C"] + 1) * (AtomNumber["N"] + 1) < length(label)) {
    if (length(formula) != 0) {
      print(
        paste(
          formula,
          ":the number of labeling species exceeded the maximum allowed number of carbon and Nitrogen isotopomers, check the input file."
        )
      )
    } else {
      print(
    ...
    for (i in 1:(AtomNumber["N"] + 1)) {
      NMatrix[, i] <-
        sapply(0:AtomNumber["N"], function(x) {
          stats::dbinom(x - i + 1, AtomNumber["N"] - i + 1, NitrogenNaturalAbundance[2])
        })
    }
    for (m in 1:(AtomNumber["N"] + 1)) {
      for (n in 1:m) {
        CNMatrix[((m - 1) * (AtomNumber["C"] + 1) + 1):(m * (AtomNumber["C"] + 1)), ((n - 1) * (AtomNumber["C"] + 1) + 1):(n * (AtomNumber["C"] + 1))] <-
          CMatrix * NMatrix[m, n]
      }
    }
    ...
    # Construct PurityMatrix
    PurityMatrix <-
      diag((AtomNumber["C"] + 1) * (AtomNumber["N"] + 1))
    CPurityMatrix <- diag((AtomNumber["C"] + 1))
    ...
    for (n in 1:(AtomNumber["N"] + 1)) {
      for (m in 1:n) {
        PurityMatrix[((m - 1) * (AtomNumber["C"] + 1) + 1):(m * (AtomNumber["C"] + 1)), ((n - 1) * (AtomNumber["C"] + 1) + 1):(n * (AtomNumber["C"] + 1))] <-
          CPurityMatrix * NPurityMatrix[m, n]
      }
    }
    ...
    Isotope.Combinations1 <-
      Isotope.Combinations %>%
      dplyr::mutate(
        NonTracerMass = .data$H2
          + .data$O17
          + .data$O18 * 2
          + .data$S33
          + .data$S34 * 2
      ...
      dplyr::filter(
        (.data$O17 + .data$O18) <= AtomNumber["O"] &
          (.data$S33 + .data$S34) <= AtomNumber["S"] &
          (.data$Si29 + .data$Si30) <= AtomNumber["Si"] &
          .data$NonTracerMass <= (AtomNumber["C"] + AtomNumber["N"])
      ) %>%
      dplyr::filter((.data$NonTracerMass == .data$C13 + .data$N15)) %>%
      dplyr::mutate(
        MassDiff = 1.00628 * .data$H2
          + 1.00422 * .data$O17
          + 2.00425 * .data$O18
          + 0.99939 * .data$S33
          + 1.99580 * .data$S34
    ...
    Isotope.Combinations2 <-
      ...
      dplyr::filter((.data$O17 + .data$O18) <= AtomNumber["O"] &
        ...
        (.data$Si29 + .data$Si30) <= AtomNumber["Si"] &
        .data$NonTracerMass <= (AtomNumber["C"] + AtomNumber["N"])) %>%
      dplyr::filter((.data$NonTracerMass == abs(.data$C13 - .data$N15)) &
        (.data$C13 * .data$N15 * .data$NonTracerMass > 0)) %>%
      dplyr::mutate(
        MassDiff =
          1.00628 * .data$H2
            + 1.00422 * .data$O17
            ...
            + 1.99705 * .data$Cl37
            + 1.99796 * .data$Br81
            - abs(1.00335 * .data$C13 - 0.99703 * .data$N15)
      ) %>%
      dplyr::filter(abs(.data$MassDiff / Charge) < Mass.Limit)
    ...
    NonTracerMatrix <-
      matrix(
        0,
        ncol = (AtomNumber["C"] + 1) * (AtomNumber["N"] + 1),
        nrow = (AtomNumber["C"] + 1) * (AtomNumber["N"] + 1)
      )
    for (m in 1:nrow(NonTracerMatrix)) {
      Cindex <- CompleteLabel[m, 1]
      Nindex <- CompleteLabel[m, 2]
      Current.Combinations <-
        Isotope.Combinations1 %>% dplyr::filter(.data$C13 == Cindex &
          .data$N15 == Nindex)
      if (nrow(Current.Combinations) != 0) {
        for (i in 1:nrow(Current.Combinations)) {
          p <-
    ...
    if (nrow(Current.Combinations) != 0) {
      for (i in 1:nrow(Current.Combinations)) {
        ...
        if (Cindex < Nindex) {
          ...
          m <-
            which(CompleteLabel$C13 == 0 & CompleteLabel$N15 == Nindex)
          for (j in 0:(AtomNumber["C"] + 1 - n)) {
          ...
          m <-
            which(CompleteLabel$C13 == Cindex & CompleteLabel$N15 == 0)
          for (j in 0:(AtomNumber["C"] + 1 - m)) {
...
dual_correction <- function(InputFile,
  ...
  for (i in 1:nrow(InputDF)) {
    if (gdata::startsWith(InputDF[[i, 1]], "Unknown")) {
  ...
  for (i in unique(InputDF$Compound)) {
    ...
    DataMatrix <-
      as.matrix(CurrentMetabolite[, 6:(ncol(CurrentMetabolite))])
    if (Isotopes == "CH") {
      Corrected.raw <- CH_Correction(
        Formula,
        DataMatrix,
        CurrentMetabolite[, 3:4],
    ...
    for (m in 1:nrow(Corrected.raw)) {
      for (n in 1:nrow(CurrentMetabolite)) {
        if (prod(Corrected.raw[m, 1:2] == CurrentMetabolite[n, 3:4]) &
          CurrentMetabolite[n, 5]) {
          Corrected <- rbind(Corrected, Corrected.raw[m, ])
        }
      }
    }
    CorrectedPercentage <-
      scale(Corrected[, 3:ncol(Corrected)],
        scale = colSums(Corrected)[3:ncol(Corrected)],
        center = FALSE
      )
    OutputMatrix <-
      rbind(OutputMatrix, Corrected[, 3:ncol(Corrected)])
    OutputPercentageMatrix <-
      rbind(OutputPercentageMatrix, CorrectedPercentage)
    OutputPoolBefore <- rbind(OutputPoolBefore, colSums(DataMatrix))
    OutputPoolAfter <-
      rbind(OutputPoolAfter, colSums(Corrected[, 3:ncol(Corrected)]))
    OutputCompound <-
      append(OutputCompound, rep(i, nrow(Corrected[, 3:ncol(Corrected)])))
  ...
  OutputPercentageDF <-
    data.frame(OutputCompound, OutputLabel, OutputPercentageMatrix)
  OutputPoolBeforeDF <-
    data.frame(OutputPoolCompound, OutputPoolBefore)
  OutputPoolAfterDF <-
    data.frame(OutputPoolCompound, OutputPoolAfter)
  if (Isotopes == "CH") {
    names(OutputDF) <-
      c("Compound", "C13", "H2", names(InputDF)[6:(length(names(InputDF)))])
  } else if (Isotopes == "CN") {
    names(OutputDF) <-
      c("Compound", "C13", "N15", names(InputDF)[6:(length(names(InputDF)))])
  }
  names(OutputPercentageDF) <- names(OutputDF)
  names(OutputPoolBeforeDF) <-
    c("Compound", names(InputDF)[6:(length(names(InputDF)))])
  names(OutputPoolAfterDF) <-
    c("Compound", names(InputDF)[6:(length(names(InputDF)))])
