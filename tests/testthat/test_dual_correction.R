...
test_that("CN correction XLSX output", {
  ...
  output_file_base <-
    tempfile(fileext = paste0(".", output_filetype))
  output_file <- paste0(
    tools::file_path_sans_ext(output_file_base),
    "_corrected.",
    output_filetype
  )
...
test_that("CN correction csv output", {
  ...
  expect_message(
    ...
    "Output written to:.*_pool_size\\.csv"
  )

  for (sheet in list("Corrected", "Normalized", "Pool size")) {
    output_file_suffix <-
      paste0("_", sub(" ", "_", tolower(sheet)), ".", output_filetype)
    output_file <-
      paste0(
        tools::file_path_sans_ext(output_file_base),
        output_file_suffix
    ...
    expect_snapshot_file(output_file,
      paste0("CN_Compound_Test", output_file_suffix),
